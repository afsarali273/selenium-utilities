...
public class UIWebElementUtil {
	public static boolean isWebElementDisplayed(WebDriver driver, By by, int waitTime, int retry) {
	...
	public static boolean isWebElementExist(WebDriver driver, By by, int waitTime, int retry) {
	...
	public static boolean isWebElementInScreen(WebDriver driver, By by, int waitTime, int retry) {
		boolean result = false;
		...
		for (int i = 0; i < retry; i++) {
			...
			String left = style.substring(style.indexOf("left:")).replace("left:", "").trim();
